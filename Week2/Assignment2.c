...
CS260
Assignment 2
Dynamic array implementation and bag interface
*/
...
void swapDynArr(DynArr *d, int i, int j) {
 /*FIX ME: You will write this function*/
    assert(d != 0);
    assert(i >= 0 && j >= 0);
    assert(i < d->size && j < d->size);
 
    double temp = d->data[i];
    d->data[i] = d->data[j];
    d->data[j] = temp;
}
...
/*FIX ME: You will write this function*/
    assert(d != 0);
    assert(pos < d->size && pos >= 0);

    /* Using pos as counter variable */
    while(pos < d->size - 1){
...
/*FIX ME: You will write this function*/
    ...
    /*addDynArr already checks preconditions and performs postconditions*/
    addDynArr(bag, val);
}
...
/*FIX ME: You will write this function*/
    ...
    if(counter < bag->size){
        ...
        while(counter < bag->size - 1){
            bag->data[counter] = bag->data[counter + 1];
